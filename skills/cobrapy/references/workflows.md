...
- Set `OUTDIR` to the requested location or a new directory in the authorized workspace. Ask only if the destination is unresolved or an existing unrelated file would be overwritten.
...
```python
OUTDIR = "cobrapy_output"  # new output directory inside the authorized workspace
```
