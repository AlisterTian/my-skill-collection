...
For plotting, honor the current Python/R request, then a language-specific input workflow, then the saved preference from `scripts/nature_figure_backend.py get`. With none available, choose a suitable installed backend using `references/backend-selection.md`, state the choice, and continue. Ask only if unresolved interoperability or runtime constraints materially affect the deliverable.

Preserve existing saved preferences. Persist a new default only when the user asks to remember it, using `scripts/nature_figure_backend.py set python` or `set r`. Do not store a task-local assumption as a user preference.
