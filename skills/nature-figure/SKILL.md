---
name: nature-figure
description: "Create, revise, and audit publication scientific figures in Python or R, including multi-panel SVG/PDF/TIFF exports and requested AI schematic drafts. Use for manuscript figures or graphical abstracts; 科研绘图、论文配图。"
---
...
Use an explicit Python/R choice first, then the existing language-specific workflow, then the saved preference from `scripts/nature_figure_backend.py get`. If none applies, choose the installed backend that fits the data and deliverable using `references/backend-selection.md`, briefly state the choice, and proceed. Ask only when interoperability or an unavailable required runtime makes the choice consequential.

Preserve saved preferences. Save a new default with `scripts/nature_figure_backend.py set python` or `set r` only when the user requests remembering that choice; a task-local choice need not become a global preference. Once selected, keep all plotting and exports in that backend. This step does not apply to the explicit AI-schematic route.
