...
Resolve the backend from the explicit request, existing language-specific workflow, or saved preference, in that order. Otherwise use the decision table and available runtimes to choose a task-local backend and proceed after a brief explanation. Ask only for a consequential unresolved interoperability or runtime choice.

Use `scripts/nature_figure_backend.py get` to read preferences. Save a new default with `set python` or `set r` only when the user asks to remember it; preserve an existing preference otherwise.
...
backends for aesthetics alone. Persist a changed preference only if the user asks to
remember it.
