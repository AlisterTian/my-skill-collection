---
name: nature-writing
description: "Draft or restructure scientific manuscript sections and first-submission materials from supplied claims and evidence, with Nature-style guidance. Use for paper arguments, Results organization, or initial submission packages; 论文写作、首次投稿。"
---
...
For `task=manuscript`, use `static/core/workflow.md` at the depth needed for the request. For direct prose or small edits, plan internally and deliver the requested text; show an outline first only when requested or when a material ambiguity prevents a supported draft.
